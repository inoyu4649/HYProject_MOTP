...
Windows판과 Linux tar.gz판은 `MOTP.exe`/`MOTP` 옆에 `mods/` 폴더를 **직접 만들고**
그 안에 넣는다. Linux DEB판은 `~/.local/share/com.hyproj.motp/mods/`(또는
`$XDG_DATA_HOME/com.hyproj.motp/mods/`)가 그 자리다. 두 가지 모양을 다 받는다:
...
### 1-1. 여러 모드를 하나의 모드팩 ZIP으로 배포하기 (v0.6.0)

ZIP 루트에 `modpack.json`을 두고, 폴더 형식의 모드를 `mods/` 아래에 나란히 넣는다.
...
```json
{
  "id": "kr.example.my-pack",
  "name": { "ko": "나의 모드팩", "en": "My Mod Pack", "ja": "私のモッドパック" },
  "version": "1.0.0",
  "icon": "icon.png"
}
```

- `id`는 원장과 완결본을 가르는 영속 신원이다. 영문/숫자/`.`/`_`/`-`만 쓴다.
- `name`은 문자열 하나 또는 언어별 객체다. `icon`은 ZIP 루트 기준 상대 경로다.
- `mods/` 컨테이너는 생략하고 각 모드 폴더를 ZIP 루트에 바로 놓아도 된다.
- 모드팩의 모드를 하나라도 끄거나 바깥 모드를 섞으면 그 실행은 **직접 만든 조합**으로 취급된다.
  게임의 모드 관리 화면에서 그 조합에 이름을 붙일 수 있다.
- 배포 모드팩은 `modpack.json.id`, 직접 만든 조합은 활성 모드 id 집합으로 원장을 가른다.
  도전과제·클리어 횟수·완결본 번호는 Vanilla 및 다른 모드팩과 섞이지 않는다.
...
```jsonc
{
  ...
  "version": "1.0.0",            // 필수
  "gameVersion": "0.7.x",        // 지원 게임 버전(아래 3절). 안 맞으면 **켜지되 경고**
  "author": "…",
  "description": { "ko": "…" },
...
한 버전이 아니라 **범위**를 적는다. 모드는 게임보다 오래 살고, 게임이 0.6.0으로 올라갈 때마다
멀쩡한 모드가 전부 경고를 달면 사람이 경고를 안 읽게 되기 때문이다.

지금 게임 버전은 **`0.7.0`**이다(타이틀 화면 왼쪽 아래에 적혀 있다). 설치와 업데이트를 HY
런처에 넘긴 판이고 게임 안의 내용은 0.6.2와 같으므로, 모딩 ID는 `mapping/v0.6.0.md`를 그대로
쓴다. `"0.6.x"`로 적은 모드도 경고 한 줄만 달고 그대로 켜진다.
...
게임 버전별 논리 ID 전체 표는 [`mapping/v0.6.0.md`](mapping/v0.6.0.md)에 있다. 모드가 어느
그림·BGM·효과음·글꼴 키를 덮을 수 있는지 소스코드를 열지 않고 이 표에서 확인할 수 있다.
...
```ts
// data/mods.ts
modPackKey(stamps)     // 직접 조합: 켜진 모드 id 집합의 해시. 없으면 null
publishedModPackKey(id) // 배포 모드팩: modpack.json.id의 해시
modPackLabel(stamps)   // 사람이 읽을 이름표: "ModA, ModB 외 2개"
```

배포 모드팩을 온전히 켜면 `modpack.json.id`가 신원이고, 일부를 끄거나 다른 모드를 섞으면 활성
모드 id 집합으로 만든 직접 조합 신원이 된다. 직접 조합에는 관리 화면에서 이름을 붙일 수 있다.
두 종류 모두 키는 버전과 순서를 보지 않는다.
...
ProfileRecord.packProgress?:     Record<string, { clears, endings, ... }> // 팩별 클리어 진행
```
...
100.00%여도 다른 쪽 숫자에 아무 영향이 없다. 모드팩 탭의 초기화 버튼은 **현재 팩의 도전과제
원장만** 비우며, 바닐라·다른 팩·완결본은 건드리지 않는다.
