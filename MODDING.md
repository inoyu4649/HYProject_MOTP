...
```jsonc
{
  ...
  "version": "1.0.0",            // 필수
  "gameVersion": "0.6.x",        // 지원 게임 버전(아래 3절). 안 맞으면 **켜지되 경고**
  "author": "…",
  "description": { "ko": "…" },
...
지금 게임 버전은 **`0.6.2`**이다(타이틀 화면 왼쪽 아래에 적혀 있다). 0.6.1에서 넓힌 Linux
플랫폼의 창 표시만 고친 판이므로 모딩 ID는 `mapping/v0.6.0.md`를 그대로 쓴다.
